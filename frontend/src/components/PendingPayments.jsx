...
const PendingPayments = () => {
    ...
    useEffect(() => {
        const fetchRecycledItemData = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:8000/api/user-pending-order-detail/${user_id}/`, {
                    method: 'GET',
                    headers: {
                ...
                const itemsWithUsernames = await Promise.all(
                    items.map(async (item) => {
                        try {
                            const userResponse = await fetch(`http://127.0.0.1:8000/api/enduser/${item.user}/`, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Authorization': `Bearer ${localStorage.getItem('access')}`,
        ...
        <>
            ...
            <div className="main-container">
                ...
                <div style={{ fontFamily: 'Arial, sans-serif', alignItems: 'center', height: '100vh', margin: 0, color: '#333' }} className='main'>
                    ...
                    <div className="container" style={{ background: '#ffffff', padding: '30px', borderRadius: '15px', boxShadow: '0 8px 16px rgba(0, 0, 0, 0.1)', width: '100%', maxWidth: '900px' }}>
                        <form>
